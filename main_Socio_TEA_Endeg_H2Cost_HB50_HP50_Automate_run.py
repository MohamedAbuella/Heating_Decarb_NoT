...
def data_import(year, scenario):
    ...
    # path_I = r'C:/Users/Mhdella/Desktop/Akhil_code/Simulation 2/Refined FES scenario inputs/'
...
def initial_run_OPGF(model, systemData, H2_prop):
    ...
    global multinet, genCapacities, Cost_H2
    ...
    multinet = run_OPGF(hour_of_day, genData,  systemData, H2_prop, temp_system, price_fg, 
                        num_simulations, heat_pumps, hydrogen_boilers, others, base_sw, 
                        HPHB_sw, EZ_ATR_FC_CHP_fg, list_ele_demand, list_gas_demand) # Running the OPGF
...
def cost_check(model, multinet, systemData):
    ...
    for name, param in {'costs': model.c, 'emissions': model.emis}.items():
    ...
    #######################
    ## Emission costs
    global emissionCost, emis_chp_gt, emis_other
...
def update_GT(year, scenario, multinet, systemData): 
    ...
    # systemData['Players']['max_p_mw'][26] = sum(multinet['nets']['power']['res_sgen']['p_mw'][:5])
    systemData['Players']['max_p_mw'][26] = 0
    systemData['Players']['max_p_mw'][5] = sum(multinet['nets']['power']['res_sgen']['p_mw'][6:])
...
def run_techno_economic_analysis(t, year, scenario, systemData, temp_system, output_dir):
    ...
    output_dir = f'Output/Res_{HPHB_mix_fg}_Price{price_fg}_EZATRFCCHP_{EZ_ATR_FC_CHP_fg}/MCP'+'/'+case_fg+'/Outs/'+str(scenario)+'/imax_'+str(eps_imax)+'_prcfg_'+str(price_fg)+'_yrfg_'+str(year_fg)+'_'+str(year)
    ...
    for H2_prop in h2_props:
        ...
        systemData = data_import(year, scenario, ws_limit)  
    ...
    for HPHB_sw in scenarios_list:
        for h2_plt in h2_proportions:
            ...
            if HPHB_sw=='Base_MC_Interv':
               ...
               if base_sw=='Base': intervention_levels = [0] 
               elif base_sw=='MC': intervention_levels = [0.1] 
               
            if HPHB_sw=='HP_Interv' or HPHB_sw=='HB_Interv':
                # intervention_levels = [0, 0.1] 
               if base_sw=='Base': intervention_levels = [1] 
               elif base_sw=='MC': intervention_levels = [1] 
               
            if HPHB_sw=='HP_HB_Interv':
                # intervention_levels = [0, 0.1] 
               if base_sw=='Base': intervention_levels = [0.5] 
               elif base_sw=='MC': intervention_levels = [0.5] 
               
               
            for HP_HB in intervention_levels:   
                
                
                if HPHB_sw=='HP_Interv':
                    heat_pump_interventions = intervention_levels
                    heat_pumps = HP_HB  # Percentage of heat pump intervention
                    hydrogen_boilers =0.1  # Percentage of hydrogen boiler intervention 
                    if HP_HB ==1:
                        hydrogen_boilers =0
                        
        
                elif HPHB_sw=="HB_Interv":
                    hydrogen_boiler_interventions = intervention_levels
                    hydrogen_boilers = HP_HB 
                    heat_pumps = 0.1 
                    if HP_HB ==1:
                        heat_pumps = 0
         
                    
                elif HPHB_sw=="HP_HB_Interv":
                    heat_pump_interventions = intervention_levels
                    hydrogen_boiler_interventions = intervention_levels
                    heat_pumps = HP_HB  
                    hydrogen_boilers = HP_HB  
                ...
                elif HPHB_sw=='Base_MC_Interv':
                    heat_pump_interventions = intervention_levels
                    hydrogen_boiler_interventions = intervention_levels
                    heat_pumps = HP_HB 
                    hydrogen_boilers = HP_HB  
                    # if HP_HB==0: base_sw = 'Base'
                    # elif HP_HB==0.1: base_sw = 'MC'
                    
                if HPHB_sw=="HP_HB_Interv" and HPHB_mix_fg=='HB_HP_70_30':
                    hydrogen_boiler_interventions = [0.7]
                    heat_pump_interventions = [0.3]
                    hydrogen_boilers = 0.7     
                    heat_pumps = 0.3
                ...
                # Adjust 'others' to ensure the total probability remains 1
            ...
            for scenario in scenarios:
                ...
                if ccs_fg == 0:
                    output_dir = f'Output/Res_{HPHB_mix_fg}_Price{price_fg}_EZATRFCCHP_{EZ_ATR_FC_CHP_fg}/MCP/{case_fg}/H_{H2_prop}/Without_CCS/Wind_limit_{ws_limit}/ext_grid_{ext_sw}/price_{price_fg}/{HPHB_sw}/Social_{base_sw}/{scenario}'
                if ccs_fg == 1:
                    output_dir = f'Output/Res_{HPHB_mix_fg}_Price{price_fg}_EZATRFCCHP_{EZ_ATR_FC_CHP_fg}/MCP/{case_fg}/H_{H2_prop}/With_CCS/Wind_limit_{ws_limit}/ext_grid_{ext_sw}/price_{price_fg}/{HPHB_sw}/Social_{base_sw}/{scenario}'
