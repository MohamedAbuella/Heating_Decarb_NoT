...
def data_source(systemData, num_simulations, H2_prop, heat_pumps, hydrogen_boilers, others):
    ...
    # time_series_profiles['g_load_time_series'] = systemData['Adjusted Gas Demand'] 
    # time_series_profiles['e_load_time_series'] = systemData['Adjusted Power Demand']
...
def gas_network(time_steps, H2_prop, temp_system):
...
def power_network(time_steps, genData, systemData):
...
def hydrogen_network(capacity_HB=0):
    
    # Create an empty hydrogen network with pure hydrogen as the fluid
    net = ppipes.create_empty_network(fluid="hydrogen")
    
    # Define junctions
    VCS1_in = ppipes.create_junction(net, pn_bar=1, tfluid_k=288.15, name="VCS1_in")
    VCS1_out = ppipes.create_junction(net, pn_bar=1, tfluid_k=288.15, name="VCS1_out")
    
    # Add a pipe to connect the junctions with an increased diameter for higher flow capacity
    ppipes.create_pipe_from_parameters(net, 
                                       from_junction=VCS1_in, 
                                       to_junction=VCS1_out, 
                                       length_km=1.0, 
                                       diameter_m=1,  # Increased from 0.1 m to 0.3 m
                                       k_mm=0.025, 
                                       name="H2_Pipe")
    
    # Add a sink for hydrogen boilers if capacity_HB is provided
    if capacity_HB > 0:
        # Convert capacity_HB (MWh) to mass flow rate (kg/s)
        # Hydrogen HHV = 39.41 kWh/kg (as defined in energy_h2_gas)
        # mdot_kg_per_s = capacity_HB (MWh) / (HHV * 3600 seconds/hour)
        mdot_kg_per_s_HB = capacity_HB / (39.41 * 3600)
        ppipes.create_sink(net, 
                            junction=VCS1_out, 
                            mdot_kg_per_s=mdot_kg_per_s_HB, 
                            name="Hydrogen Boiler")
        
        
    ppipes.create_ext_grid(net, junction=1, p_bar=70, t_k=288.15)
...
def coupling_chp(net_power, net_gas, multinet, H2_prop):
    # create elements corresponding to conversion units
    # CHP units
    # Gas-to-power 1
...
def coupling_gfg(net_power, net_gas, multinet, H2_prop):
    
    EF=mdot_energy_conv(H2_prop)
    
    # create elements corresponding to conversion units
    # GFG units as continuation of CHPs
    # Gas-to-power 3
    # global g2p_3_gas, g2p_3_el 
    # capacities = [30,30,50] # in MW
    capacities = [0,0,0]
    
    g2p_3_gas = ppipes.create_sink(net_gas, junction=11, mdot_kg_per_s=(capacities[0] * 1000*0.85)/(EF*3600), name="GFG1 consumption")
    g2p_3_el = ppower.create_sgen(net_power, bus=179, p_mw=0, 
                                  min_p_mw=0, max_p_mw=capacities[0], name="GFG1 feed in", controllable=True)
    g2p_3_ctrl = G2PControlMultiEnergy(multinet, g2p_3_el, g2p_3_gas, efficiency=0.85,
                                 name_power_net="power", name_gas_net="gas")
    
    # # Gas-to-power 4
    # global g2p_4_gas, g2p_4_el 
    g2p_4_gas = ppipes.create_sink(net_gas, junction=11, mdot_kg_per_s=(capacities[1] * 1000*0.85)/(EF*3600),  name="GFG2 consumption")
    g2p_4_el = ppower.create_sgen(net_power, bus=179, p_mw=0, 
                                  min_p_mw=0, max_p_mw=capacities[1], name="GFG2 feed in", controllable=True)
    g2p_4_ctrl = G2PControlMultiEnergy(multinet, g2p_4_el, g2p_4_gas, efficiency=0.85,
                                  name_power_net="power", name_gas_net="gas")
    
    # # Gas-to-power 5
    # global g2p_5_gas, g2p_5_el 
    g2p_5_gas = ppipes.create_sink(net_gas, junction=12, mdot_kg_per_s=(capacities[2] * 1000*0.85)/(EF*3600), name="GFG3 consumption")
    g2p_5_el = ppower.create_sgen(net_power, bus=122, p_mw=0, 
                                  min_p_mw=0, max_p_mw=capacities[2], name="GFG3 feed in", controllable=True)
    g2p_5_ctrl = G2PControlMultiEnergy(multinet, g2p_5_el, g2p_5_gas, efficiency=0.85,
                                  name_power_net="power", name_gas_net="gas")
...
def fuel_cell(net_power, net_hyd, multinet, FC_capacity_value, H2_prop):
    
    capacity = FC_capacity_value # Capacity in MW
    
    # # Fuel cell can be modelled as a G2P
    fuel_cell_in  = ppipes.create_sink(net_hyd, junction = 1, mdot_kg_per_s=0, max_p_mw=capacity, name='Fuel_cell_feed_in')
    fuel_cell_out = ppower.create_sgen(net_power, bus=0, p_mw=capacity, 
                                        min_p_mw=0, name='fuel_cell_output',controllable=False)
    fuel_cell_ctrl = G2PControlMultiEnergy(multinet, fuel_cell_out, fuel_cell_in, efficiency=0.65,
                                            name_power_net= 'power', name_gas_net='hydrogen', calc_gas_from_power=True)
    
    return fuel_cell_out
...
def form_multinet(net_power, net_gas, net_hyd, H2_prop):
    ...
    coupling_chp(net_power, net_gas, multinet, H2_prop)
    coupling_gfg(net_power, net_gas, multinet, H2_prop)
...
def OPGF(systemData, multinet, genData, net_power, net_gas, net_hyd, H2_prop, price_fg, 
    ...
    if base_sw=='Base':
        print('g_demand_Base=', g_demand)
        print('e_demand_Base=', e_demand)
    if base_sw=='MC':
    ...
    g2p_limits = [multinet.nets['gas'].sink['mdot_kg_per_s'][15]*(EF*3600)/(0.4*1000),
                  multinet.nets['gas'].sink['mdot_kg_per_s'][16]*(EF*3600)/(0.4*1000),
                  multinet.nets['gas'].sink['mdot_kg_per_s'][17]*(EF*3600)/(0.4*1000),
                  multinet.nets['gas'].sink['mdot_kg_per_s'][18]*(EF*3600)/(0.4*1000),
...
def run_OPGF(t, genData, systemData, H2_prop, temp_system, price_fg, num_simulations,
    ...
    ### To avoide opgf didn't converge ###
    if HPHB_sw == 'Base_MC_Interv':
        genData['8']=genData['8']*3
        
        ### To avoide opgf didn't converge ###
    if HPHB_sw == 'HP_Interv':
        genData['8']=genData['8']*3
        
    ### To avoide opgf didn't converge ###    
    if HPHB_sw == 'HP_HB_Interv':
        genData['8']=genData['8']*2
        
    ### To avoide opgf didn't converge ###    
    if HPHB_sw == 'HB_Interv':
        genData['8']=genData['8']*3
    ...
    for i in time_steps:
        ...
        net_power = power_network(i, genData, systemData)
        net_hyd = hydrogen_network()
        global multinet
        multinet = form_multinet(net_power, net_gas, net_hyd, H2_prop)
