...
def data_import(year, scenario, ws_limit):
    ...
    ) = trend_social_scale(path, year, scenario)
    ...
    scaled_social_power_demand = social_power_demand['Base load Electrcity Demand'] * power_FES_trend
    scaled_social_gas_demand = social_gas_demand_MWh['Base load Gas Demand'] * gas_FES_trend

    # Distribute the scaled social demands across the nodes based on original node proportions
    ...
    # Handle any NaN values resulting from zero total demand at any hour
...
def trend_social_scale(path, year, scenario):
    # Load the fixed demand profiles from 'Social Interventions.xlsx'
    social_power_demand = pd.read_excel(path + 'Social Interventions.xlsx',sheet_name='Electrical Demand Profile',index_col=0)
    ...
    total_social_power = social_power_demand['Base load Electrcity Demand'].sum()
    total_social_gas = social_gas_demand_MWh['Base load Gas Demand'].sum()
...
if __name__ == "__main__":
    ...
    systemData = (year, scenario, ws_limit)
